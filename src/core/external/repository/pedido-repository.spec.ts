...
describe("PedidoRepository", () => {
  ...
  it("deve salvar um pedido corretamente", async () => {
    ...
    expect(itemPedidoRepoMock.save).toHaveBeenCalledWith(expect.objectContaining({ idPedido: "123" }));
  ...
  it("deve atualizar o status de um pedido", async () => {
    ...
    const result = await pedidoRepository.atualizarStatusPedido("1", atualizarStatusPedidoDTO);
  ...
  it("deve buscar um pedido pelo ID do pagamento", async () => {
    const mockPedido = { id: "1", idPagamento: "pag123", combo: [] } as PedidoEntity;
